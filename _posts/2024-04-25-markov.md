...
While working with Markov chains, let _{Xt, t = 0, 1, 2,...}_ be a stochastic process. The state of a Markov chain at time _t_ is the value of _Xt_. For example, if _Xt = i_, then the process is said to be in state _i_ at time _t_. The state-space, S, is the set of all possible states. To summarize, if _S_ is a countable set, {i,j,k,...}, each _i∈S_ is called a state and _S_ is called the state-space. Recalling the Markov property we can say _Xt+1_ depends on the current state _Xt_, but not _Xt−1,...,X1,X0_.
